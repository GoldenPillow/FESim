...
**출처**: `RandomSeed.Initialize` 0x2375000 · `Random.Peek` 0x2375060 · `Random.Spin` 0x2375080 · `Random.GetMinMax` 0x23751B0 · `Random.GetMaxMin` 0x2375240 · `Random.GetF01` 0x23750F0 · `Random.IsProbability100` 0x23754B0 · `Random.GetIndex` 0x2375520
...
**출처**: `GetAttackScore` 0x19561E0 · `GetAttackPosition` 0x193CB30 · `CheckAttackPriorityImpl` 0x1955ED0 · `CompareAttackPriorityWithBlow` 0x19552D0
...
**출처**: `undefined` undefined · `undefined` undefined · `undefined` undefined
  ...
  "contracts": [
    ...
    },
    {
      ...
      "cases": [
        ...
        "★requireHit 게이트 — 명중 0인 방해는 후보에서 빠진다(공격 경로와 같은 게이트)",
        "★자군(0)과 우군 NPC(2)는 같은 배치에서 같은 수를 둔다 — 좌표·순번·난수 통제",
        "적(1) 턴은 dispos 突撃 그대로 — 정본이 의도한 비대칭까지 지우지 않는다",
        "★Opposition 챕터(동맹표 [0,1,2])에서는 우군 NPC도 慎重이 아니다 — 게이트는 force가 아니라 동맹표다",
        "★IsClever 진영은 dispos 플래그가 없어도 대미지 0 후보를 버린다",
        "적(1) 턴은 dispos `ZeroAttack`(4) 플래그가 있을 때만 버린다",
        "★Lua 해제(`AiSetRejectPower0Attack(u,false)`)가 루나틱 강제를 이긴다",
        "Lua가 켜면 난이도·플래그가 없어도 걸린다(인자 true — 대칭 확인)",
        "마지막 호출이 이긴다 — 같은 유닛에 두 번 걸리면 뒤가 정본이다",
        "★베이스 지형의 PlayerDefense/EnemyDefense는 무시된다",
        "오버레이 지형의 사이드항은 그대로 더해진다 — 층만 갈렸다",
        "★관통 — 베이스 사이드항이 발판 선택을 흔들지 않는다(위치 스코어까지)",
        "★채택 스코어·후보·쓰인 레이아웃(강제 여부)·페이즈 스텝이 결정에 실려 나온다",
        "IsClever 진영이면 레이아웃이 강제됐다는 것까지 나온다",
        "기각된 표적은 사유(게이트 이름)와 함께 남는다"
      ]
    }
  ]
}
...
```json
{
  ...
  "entries": [
    ...
    },
    {
      ...
      "status": "deferred",
      "evidence": "AiSetSequence(코퍼스 369회 — 게임플레이 API 최다)·AiSetActive·AiSetRejectPower0Attack·AiClearMoveLimit = 유닛 aiScript에 원문 인자 기록 + ai 이벤트(결정 2026-08-18: 소비 = MP4 AI 실행기) · ⚠AiGetActive 질의는 false 강하(미소비 상태 — MP4에서 실상태로 교체) · ★부분 소비 개시(2026-08-22): `AiSetRejectPower0Attack`만 배선했다 — engine ai/unit.ts `rejectsPower0`가 유닛 aiScript의 **마지막 호출을 난이도 강제·dispos 플래그보다 우선**한다. 정본이 루나틱 강제를 Lua로 되돌리는 이유가 스크립트에 적혀 있다(ハマり = 교착). 코퍼스 전수 = e001(루나틱·적 전원)·m020(보스)·g002(증원 PID 7종)·common(비행 적, ☠호출자가 common_p0뿐이라 챕터 밖) · g005는 주석 사문. 관통 테스트 = engine tests/events.test.ts(Lua → aiScript → rejectsPower0). ☠나머지 3종(Sequence·Active·ClearMoveLimit)은 여전히 기록만"
    }
  ]
}
```
...
☠**미생성** — 선행 = items.json의 UseType·Enhance 사영과 chapternotes 상점 확장(MP8 A8 §5 산출 스키마). 파이프라인이 아직 안 싣는다

## 전략 규약

☠**미생성** — 선행 = C 전략 엔진(packages/engine/src/strategy/). 코드가 없으므로 뽑을 것이 없다 — ☠산문으로 채우면 이중화다
