...
export const FIDELITY_CATEGORIES: readonly FidelityCategory[] = [
  { id: "movement", label: { en: "Movement", ja: "移動", ko: "이동" } },
  { id: "actions", label: { en: "Actions", ja: "行動", ko: "행동" } },
  { id: "combat", label: { en: "Combat", ja: "戦闘", ko: "전투" } },
  { id: "units", label: { en: "Units & Stats", ja: "ユニット・能力値", ko: "유닛·스탯" } },
  { id: "skills", label: { en: "Skills", ja: "スキル", ko: "스킬" } },
  { id: "emblem", label: { en: "Emblems & Engage", ja: "紋章士・エンゲージ", ko: "엠블렘·인게이지" } },
  { id: "weapons", label: { en: "Weapons & Items", ja: "武器・アイテム", ko: "무기·아이템" } },
  { id: "turn", label: { en: "Turn Structure", ja: "ターン構造", ko: "턴 구조" } },
  { id: "ai", label: { en: "Enemy AI Data", ja: "敵AIデータ", ko: "적 AI 데이터" } },
  { id: "events", label: { en: "Chapter Events (Lua)", ja: "章イベント(Lua)", ko: "챕터 이벤트(Lua)" } },
];
...
export const FIDELITY: readonly FidelityEntry[] = [
  // ── 이동 ──
  {
    ...
    label: { en: "Movement range from terrain cost (255 = impassable)", ja: "地形コストによる移動範囲(255 = 進入不可)", ko: "지형 코스트 이동 범위(255 = 진입 불가)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "One move per activation", ja: "1行動につき移動1回", ko: "활성화당 이동 1회" },
    status: "anchored",
    evidence: "베타 실기 발견(2026-08-16) · battle.test.ts 활성화당 이동 1회",
  },
  {
    id: "movement.canter-distance",
    label: { en: "Canter: N tiles after acting (skill Removable)", ja: "再移動: 行動後Nマス(skills.json Removable)", ko: "재이동: 행동 후 N칸(skills.json Removable)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Canter obeys terrain cost", ja: "再移動にも地形コストを適用", ko: "재이동에 지형 코스트 적용" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Cannot pass through non-allied units", ja: "非同盟軍のユニットは通過不可", ko: "비동맹 진영 통과 불가" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Pass through allied forces, cannot stop on them", ja: "同盟軍は通過可・停止不可", ko: "동맹 진영 통과 가능·정지 불가" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Enemy and third force block each other", ja: "敵軍↔友軍は相互に通過不可", ko: "적군↔우군 상호 차단" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Move is provisional until an action commits it", ja: "行動確定までは移動が仮(再配置自由・原点で取消)", ko: "행동 확정 전 잠정 이동(자유 재배치·원점 취소)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Structures (doors, walls) affect passability", ja: "構造物(扉・壁)の通行判定", ko: "구조물(문·벽) 통행 반영" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Multi-tile units (BmapSize)", ja: "複数マス占有ユニット(BmapSize)", ko: "다칸 유닛(BmapSize)" },
    status: "absent",
  ...
  {
    id: "movement.warp",
    label: { en: "Warp and other staff/item movement", ja: "杖・アイテムによる移動(ワープ等)", ko: "지팡이·아이템 이동(워프 등)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Departure-tile movement bonus (MoveFirst)", ja: "出発マスの移動力補正(MoveFirst)", ko: "출발 칸 이동력 보정(MoveFirst)" },
    status: "implemented",
    evidence:
  ...
  {
    id: "movement.zoc",
    label: { en: "Zone of control (skill-driven only)", ja: "支配領域(ZOC — スキル由来のみ)", ko: "제어영역(ZOC — 스킬 전용)" },
    status: "absent",
    evidence:
  ...
  {
    id: "actions.wait",
    label: { en: "Wait ends the unit's action", ja: "待機 = 行動終了", ko: "대기 = 행동 완료" },
    status: "implemented",
    evidence: "battle.test.ts",
  },
  {
    id: "actions.attack",
    label: { en: "Attack command and battle resolution", ja: "攻撃コマンド・戦闘処理", ko: "공격 명령·전투 해결" },
    status: "anchored",
    evidence: "corpus.test.ts(M002·M003 실기 예보 일치)",
  },
  {
    id: "actions.staff",
    label: { en: "Staves (heal, warp, status)", ja: "杖(回復・ワープ・状態異常)", ko: "지팡이(회복·워프·상태)" },
    status: "implemented",
    evidence:
  ...
  {
    id: "actions.items",
    label: { en: "Item use (vulneraries etc.)", ja: "アイテム使用(傷薬等)", ko: "아이템 사용(회복약 등)" },
    status: "implemented",
    evidence:
  ...
  {
    id: "actions.trade",
    label: { en: "Trade and convoy", ja: "交換・輸送隊", ko: "교환·수송대" },
    status: "implemented",
    evidence:
  ...
  {
    id: "actions.interact",
    label: { en: "Chests, villages, doors", ja: "宝箱・民家・扉の操作", ko: "상자·민가·문 상호작용" },
    status: "deferred",
    evidence:
  ...
  {
    id: "actions.talk",
    label: { en: "Talk command", ja: "会話コマンド", ko: "대화 커맨드" },
    status: "deferred",
    evidence: "TalkMenuItem·Label.Talk=38(MAP_COMMANDS §1-3) — 트리거 = 맵 이벤트/Lua 소관이라 interact와 함께 MP2 이월",
  },
  {
    id: "actions.dance",
    label: { en: "Dance (grant another action)", ja: "踊り(再行動の付与)", ko: "춤(재행동 부여)" },
    status: "implemented",
    evidence:
  ...
  {
    id: "actions.engage",
    label: { en: "Engage activation command", ja: "エンゲージ発動コマンド", ko: "인게이지 발동 명령" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage attack (art) command", ja: "エンゲージ技(攻撃技)", ko: "인게이지 기술(공격기)" },
    status: "assumed",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage sub-commands (rewarp/rod/charge/wait/summon)", ja: "エンゲージ下位コマンド(リワープ・ロッド・チャージ・待機・召喚)", ko: "인게이지 세부기(리워프·로드·차지·대기·소환)" },
    status: "absent",
    evidence:
  ...
  {
    id: "actions.destroy",
    label: { en: "Destroy terrain/structures", ja: "破壊(構造物を壊す)", ko: "파괴(구조물 부수기)" },
    status: "anchored",
    evidence:
  ...
  {
    id: "actions.cannon",
    label: { en: "Cannon (fire from terrain)", ja: "大砲(砲撃)", ko: "대포(포격)" },
    status: "absent",
    evidence:
  ...
  {
    id: "actions.torch",
    label: { en: "Torch (fog vision)", ja: "たいまつの点灯/消灯", ko: "횃불 켜기/끄기" },
    status: "deferred",
    evidence: "TorchOnMenuItem + terrain.json Command(TorchOn=1/Off=2)(MAP_COMMANDS §1-3) — 선행 = 안개/시야 시스템(미배선)",
  },
  {
    id: "actions.guard",
    label: { en: "Chain guard assignment", ja: "チェインガードの指定", ko: "체인가드 지정" },
    status: "implemented",
    evidence:
  ...
  {
    id: "actions.enchant",
    label: { en: "Enchant item/weapon", ja: "エンチャント(強化付与)", ko: "인챈트(강화 부여)" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Skill-granted map commands", ja: "スキル由来のマップコマンド(汎用)", ko: "스킬 파생 맵 커맨드(범용)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Transporter station", ja: "輸送ステーション(部隊編成)", ko: "수송정거장" },
    status: "deferred",
    evidence: "TransporterMenuItem(MAP_COMMANDS §1-1) — 부대 편성 변경 시설. 선행 = 캠페인층(MP5)·해당 시설 실재 맵",
  },
  {
    id: "actions.god-change",
    label: { en: "God (emblem ring) change on map", ja: "紋章士(指輪)変更コマンド", ko: "문장(반지) 변경 커맨드" },
    status: "deferred",
  ...
  // ── 전투 ──
  {
    ...
    label: { en: "Forecast numbers from Calculator.xml DSL", ja: "戦闘予測の数値 = calculator.xml DSLを直接実行", ko: "예보 수치 = calculator.xml DSL 직접 실행" },
    status: "anchored",
    evidence:
  ...
  {
    id: "combat.true-hit",
    label: { en: "True hit model (displayed 50+ = sin hybrid)", ja: "命中の実確率(表示50以上 = sinハイブリッド)", ko: "명중 실확률(표시 50 이상 = sin 하이브리드)" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Critical = 3x damage", ja: "必殺 = ダメージ3倍", ko: "필살 = 데미지 3배" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Damage integerization points (truncate toward zero)", ja: "ダメージの整数化地点(0方向へ切り捨て)", ko: "대미지 정수화 지점(0 방향 절삭)" },
    status: "anchored",
    evidence:
  ...
  {
    id: "combat.crit-rng",
    label: { en: "General probability check (linear, 0.001% step)", ja: "一般確率判定(必殺・発動 — 線形・0.001%刻み)", ko: "일반 확률 판정(필살·발동 — 선형·0.001% 해상도)" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Forecast/AI simulation is deterministic (RNG bypass)", ja: "戦闘予測・AIシミュは決定論的(乱数バイパス)", ko: "예보·AI 시뮬은 결정론(난수 우회)" },
    status: "anchored",
  ...
  {
    id: "combat.follow-up",
    label: { en: "Follow-up attack from calculator formula", ja: "追撃判定 = calculatorの公式", ko: "추격 판정 = calculator 공식" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Follow-up capped by remaining weapon durability", ja: "追撃は武器の残り耐久が上限", ko: "추격의 잔여 내구 상한" },
    status: "absent",
    evidence:
      "★IL2CPP 코드 확정(5.0.0, 2026-08-17, il2cpp/SEQUENCE_BREAK.md §2-8·§3): CalcBattleTimesImpl 최종 단계가 min(追撃条件 ? 2 : 1, UnitItem.Endurance)를 취한다(RVA 0x1E88BBC) — **잔여 내구 1이면 공속차가 5 이상이어도 추격이 없다** · 엔진은 내구 개념이 없어 미발현(선행 = 무기 내구 모델) · ★2단계 배선(2026-08-19 MP8 G1, _mp8/G1_battletimes_order.md·G1_giveskill_layer.md)): 오더 큐 배선 때 재확인 — `BattleWeapon`(packages/shared/src/battle.ts)에 내구 필드가 없다(`uses`는 StaffItem·ConsumableItem 전용) ⇒ **추측으로 채우지 않고 그 항을 뺐다**. ★제거 조건 = 무기 내구 모델이 서면 formula/combat.ts `baseBattleTimes` 반환에 `Math.min(base, 잔여 내구)`를 넣는다(코드 주석에도 같은 문구)",
  },
  {
    id: "combat.counter-range-gate",
    label: { en: "Counter-attack eligibility gates", ja: "反撃可能条件(射程・状態・武器)", ko: "반격 가능 조건(사거리·상태·무기)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Interrupt order (Break follow-up)", ja: "割込みオーダー — ブレイク時の追撃", ko: "割込み(가로채기) 오더 — 브레이크 시 추격" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Chain attack accuracy: base 80, skills override via '='", ja: "チェインアタック命中率 = 基本80固定、スキルが=演算で上書き", ko: "체인 어택 명중률 = 기본 80 고정, 스킬이 = 연산으로 덮어씀" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Chain attack damage taken cut to 20%", ja: "チェインアタックの被ダメージ = 20%に減衰", ko: "체인 어택 받는 대미지 = 20%로 감쇠" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Strike order: chain, attack, counter, follow-ups", ja: "攻撃順: チェイン→本攻撃→反撃→追撃→敵追撃", ko: "타격 순서: 체인→본공격→반격→추격→적추격" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Multi-strike per engagement turn (attack count 2+)", ja: "攻撃回数(2以上)", ko: "타격 횟수(攻撃回数 2 이상)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engagement turn count model", ja: "手番回数(交戦ターン数)モデル", ko: "手番回数(교전 턴 수) 모델" },
    status: "implemented",
  ...
  },
  {
    ...
    label: { en: "Order-altering flow skills (Vantage etc.)", ja: "順序変更系スキル(待ち伏せ等)", ko: "순서 변경 흐름 스킬(待ち伏せ 등)" },
    status: "absent",
    evidence:
  ...
  {
    id: "combat.advantage",
    label: { en: "Weapon triangle (sword>axe>lance, arts>bow/knife/tome)", ja: "相性(剣>斧>槍>剣・体術>弓/短剣/魔道書)", ko: "상성(검>도끼>창>검·체술>활/단검/마도서)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Advantage-conditional skills", ja: "相性条件スキル(武器相性激化・相性激化)", ko: "상성 조건 스킬(武器相性激化·相性激化)" },
    status: "absent",
    evidence: "combatEnv에 武器相性 변수 부재 = 전부 무발동(안전 강하) · ActNames 시점 해석 미확정(부호 반전 위험, gaps/A §7-1) — 실기 표본 선행",
  },
  {
    id: "combat.break",
    label: { en: "Break: advantage + hit forfeits counters", ja: "ブレイク: 相性有利+命中で反撃を奪う", ko: "브레이크: 상성 유리+명중 = 반격 몰수" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Break immunity (armored style, null skill)", ja: "ブレイク無効(重装スタイル・無効スキル)", ko: "브레이크 면역(중장 스타일·무효 스킬)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Break recovery: after one defended combat, or own phase start", ja: "ブレイク解除 = 被攻撃戦闘1回の直後、または自軍フェイズ開始時", ko: "브레이크 해제 = 피격 전투 1회 직후 또는 자기 군 페이즈 시작" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Chain attack (backup style in range)", ja: "チェインアタック(連携スタイル・射程内から参加)", ko: "체인어택(연계 스타일·사거리 내 협공)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Chain guard", ja: "チェインガード", ko: "체인가드" },
    status: "assumed",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage guard", ja: "エンゲージガード", ko: "엔게이지 가드" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage-technique damage is a distinct type (separately mitigable)", ja: "エンゲージ技のダメージ = 独立した種別(個別に軽減対象)", ko: "인게이지 기술 대미지 = 독립 유형(별도 감쇠 대상)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "combat.smash",
    label: { en: "Smash weapons (knockback, no first strike)", ja: "スマッシュ武器(押し出し・先制不可)", ko: "스매시 무기(밀치기·선공 불가)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Weight-over-build damage bonus (physical, cap +5)", ja: "重さ超過ダメージ = min(武器の重さ-体格, 5)、物理限定", ko: "무게 초과 대미지 = min(무기 무게-체격, 5), 물리 한정" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Effectiveness (armored, cavalry, flying, dragon)", ja: "特効(重装・騎馬・飛行・竜族等)", ko: "특효(중장·기병·비병·용족 등)" },
    status: "implemented",
  ...
  },
  {
    ...
    label: { en: "Terrain avoid/defense bonuses", ja: "地形の回避・守備補正", ko: "지형 회피·방어 보정" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Effectiveness immunity/negation", ja: "特効無効(加護・バリア)", ko: "특효 무효(가호·배리어)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Force-asymmetric terrain modifiers", ja: "自軍/敵軍で非対称な地形補正(瘴気等)", ko: "자군/적군 비대칭 지형 보정(瘴気 등)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Support (bond) bonuses (adjacent only)", ja: "支援(絆)補正 — 隣接1マスのみ", ko: "지원(인연) 보정 — 인접 1타일" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Status effects (poison, freeze, ...)", ja: "状態異常(毒・フリーズ等)", ko: "상태이상(독·동결 등)" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Staff statuses: freeze, silence, stun (apply/duration/gates)", ja: "杖の状態異常 — 移動不可・沈黙・スタン(付与・持続・ゲート)", ko: "지팡이 상태이상 — 이동불가·침묵·기절(부여·지속·게이트)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Fang curse: max HP -5 per stack, 4 discrete tiers", ja: "牙の呪い — 最大HP-5ずつ4段階(-5/-10/-15/-20)", ko: "송곳니의 저주 — 최대 HP -5씩 4단(-5/-10/-15/-20)" },
    status: "anchored",
  ...
  {
    id: "combat.staff-hit",
    label: { en: "Offensive staff hit/avoid", ja: "妨害杖の命中・回避", ko: "방해 지팡이 명중·회피" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Range-based hit falloff (-10 per tile from range 4)", ja: "遠距離の命中減衰(射程4から1マスごとに-10)", ko: "원거리 명중 감쇠(4칸부터 칸당 -10)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "combat.hp-stock",
    label: { en: "Boss HP stocks (multi-phase revival)", ja: "ボスのHPストック(多段復活)", ko: "보스 HP 스톡(다단부활)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Script-injected combat modifiers", ja: "スクリプト注入の戦闘補正", ko: "스크립트 주입 전투 보정" },
    status: "absent",
    evidence: "Lua UnitCommandPrepare 명중 보정 주입(m000/m001) · BattleAfter 보스 AI 전환(m017)(gaps/M §3)",
  },
  {
    id: "combat.kill-bonus",
    label: { en: "Kill bonus drops", ja: "撃破ボーナスのドロップ", ko: "격파 보너스 드롭" },
    status: "absent",
    evidence: "killbonus.xml 격파/피격파 드롭 확률 테이블 — 파이프라인·엔진 소비 0, dispos Item.Drop과 별개 계층(gaps/L)",
  },
  {
    id: "combat.exp",
    label: { en: "EXP from calculator formulas and tables", ja: "経験値 = calculatorの原文公式+テーブル", ko: "경험치 = calculator 원문 공식+테이블" },
    status: "assumed",
    evidence:
  ...
  },
  {
    ...
    label: { en: "EXP table out-of-domain = boundary clamp", ja: "経験値テーブルの定義域外 = 境界クランプ", ko: "경험치 테이블 정의역 밖 = 경계 클램프" },
    status: "assumed",
    evidence: "정의역 = 레벨차 -39..+40 · 34/35 테이블은 양끝 상수 평탄 — 위험은 補助レベル差減衰値 마이너스 끝 선형 하나뿐(gaps/A §3)",
  },
  {
    id: "combat.exp-staff",
    label: { en: "Staff EXP", ja: "杖の経験値", ko: "지팡이 경험치" },
    status: "anchored",
    evidence:
  ...
  {
    id: "combat.exp-dance",
    label: { en: "Dance EXP", ja: "踊りの経験値", ko: "춤 경험치" },
    status: "absent",
    evidence: "踊り経験計算 = clamp(踊り基本値(레벨+내부레벨)+補助レベル差減衰値,1,100)(gaps/A §0-2)",
  },
  {
    id: "combat.exp-chain-guard",
    label: { en: "Chain guard EXP", ja: "チェインガードの経験値", ko: "체인가드 경험치" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Summon EXP", ja: "召喚の経験値", ko: "소환 경험치" },
    status: "absent",
    evidence: "召喚経験計算 = clamp(召喚基本値(레벨+내부레벨),1,100) — 레벨차 무관(gaps/A §0-2)",
  },
  {
    id: "combat.exp-enchant",
    label: { en: "Enchant EXP", ja: "エンチャントの経験値", ko: "인챈트 경험치" },
    status: "absent",
    evidence: "エンチャント経験計算 = clamp(エンチャント基本値(레벨+내부레벨),1,100)(gaps/A §0-2)",
  },
  {
    id: "combat.exp-arena",
    label: { en: "Arena EXP branch", ja: "闘技場の経験値分岐", ko: "투기장 경험치 분기" },
    status: "absent",
    evidence: "戦闘経験計算 3분기 중 투기장 2분기 미소비(闘技場中·クリア済み 하드코딩 0)(gaps/A §0-2)",
  },
  {
    id: "combat.levelup-growth",
    label: { en: "Level-up growth (cap gate, retry up to 4)", ja: "レベルアップ成長 — 上限ゲート・最大4回まで再抽選", ko: "레벨업 성장 — 상한 게이트·최대 4시도 재굴림" },
    status: "anchored",
  ...
  {
    id: "combat.max-level",
    label: { en: "Max level stops EXP", ja: "最大レベル = 経験値停止", ko: "최대 레벨 = 경험치 정지" },
    status: "anchored",
    evidence:
  ...
  // ── 유닛·스탯 ──
  {
    ...
    label: { en: "Stat derivation model", ja: "能力値算出モデル(兵種Base+Offset+成長)", ko: "스탯 산출 모델(직업Base+Offset+성장)" },
    status: "anchored",
    evidence: "SerenesForest 36명×9스탯 전수 일치 · stats.test.ts",
  },
  {
    id: "units.stat-derivation-edge",
    label: { en: "Stat cap/floor and AutoGrowOffset handling", ja: "能力値の上限(job+person Limit)・下限0・AutoGrowOffset", ko: "스탯 상한(job+person Limit)·하한 0·AutoGrowOffset" },
    status: "assumed",
  ...
  },
  {
    ...
    label: { en: "Internal level cap on promotion", ja: "内部レベル上限(クラスチェンジ時)", ko: "내부 레벨 상한(전직 시)" },
    status: "absent",
    evidence: "内部レベル計算 = clamp(内部レベル+레벨-1, 0, 난이도별 50/40/30) — 엔진 미호출(gaps/A §0-2)",
  },
  {
    id: "units.move-enhance",
    label: { en: "Move stat bonuses (EnhanceValue.Move)", ja: "移動力補正(EnhanceValue.Move)", ko: "이동력 보정(EnhanceValue.Move)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Per-difficulty skill sets (Normal/Hard/LunaticSids)", ja: "難易度別スキル(Normal/Hard/LunaticSids)", ko: "난이도별 스킬(Normal/Hard/LunaticSids)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Battle-style granted skills", ja: "戦闘スタイル付与スキル(隠密・魔法等)", ko: "전투 스타일 부여 스킬(은밀·마법 등)" },
    status: "absent",
    evidence:
  ...
  {
    id: "units.job-skills",
    label: { en: "Job skills (innate/learning/lunatic)", ja: "兵種スキル(Skills・LearningSkill・LunaticSkill)", ko: "병과 스킬(Skills·LearningSkill·LunaticSkill)" },
    status: "absent",
    evidence:
  ...
  {
    id: "units.job-growth",
    label: { en: "Generic enemy growth from job", ja: "一般敵の成長ソース(job.BaseGrow)", ko: "일반 적 성장 소스(job.BaseGrow)" },
    status: "absent",
  ...
  {
    id: "units.promotion",
    label: { en: "Class promotion tree", ja: "クラスチェンジ体系(HighJob/LowJob)", ko: "전직 체계(HighJob/LowJob)" },
    status: "absent",
    evidence: "전직 트리 정식화(LowJob은 JID 아닌 MSBT 라벨, gaps/H) · 내부 레벨 상한은 units.internal-level-cap",
  },
  {
    id: "units.skirmish-generation",
    label: { en: "Skirmish enemy generation", ja: "遭遇戦の敵生成(encount.xml)", ko: "조우전 적 생성(encount.xml)" },
    status: "absent",
    evidence: "★서비스 피쳐 제외(사용자 결정 2026-08-17) — 데이터 정식화만 보존(무기등급 17·직업풀 25·골드 18, gaps/K §6)",
  },
  {
    id: "units.meal-buff",
    label: { en: "Meal stat buffs (Somniel cooking)", ja: "食事バフ(料理による能力値補正)", ko: "식사 버프(요리 스탯 보정)" },
    status: "absent",
    evidence: "cook.xml 出来栄え/料理 Enhance 필드(Str~Mdef s8) — 출격 전 보정 기전, 장부 밖이었음(gaps/O)",
  },
  {
    id: "units.difficulty-scaling",
    label: { en: "Per-difficulty levels and stats from dispos", ja: "難易度別のレベル・能力値(dispos・Offset)", ko: "난이도별 레벨·스탯(dispos·Offset)" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Divine Paralogue enemy level scaling (runtime)", ja: "神竜の章(g001~g006)の敵レベル実行時スケーリング", ko: "신룡의 장(g001~g006) 적 레벨 런타임 스케일링" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Per-chapter preset party state (level, promotion, inventory, sync)", ja: "章ごとのプリセット部隊状態(レベル・クラスチェンジ・持ち物・シンクロ)", ko: "챕터별 프리셋 파티 상태(레벨·승급·소지품·싱크로)" },
    status: "assumed",
  ...
  },
  {
    ...
    label: { en: "Equipped weapon = first attack weapon in inventory", ja: "装備武器 = 持ち物の最初の攻撃武器", ko: "장비 무기 = 소지품 첫 공격 무기" },
    status: "assumed",
  ...
  // ── 스킬 ──
  {
    ...
    label: { en: "Static stat bonuses (EnhanceValue)", ja: "静的な能力値補正(EnhanceValue)", ko: "정적 스탯 보정(EnhanceValue)" },
    status: "anchored",
    evidence: "corpus.test.ts 기본능력 표시 일치(싱크로 칩 포함)",
  },
  {
    id: "skills.act-values",
    label: { en: "Combat value modifiers (ActNames DSL)", ja: "計算値補正(ActNames DSL — 小数保持・表示は切り捨て)", ko: "계산값 보정(ActNames DSL — 소수 유지·표시 내림)" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Skill activation filters (Stand/Action/Timing)", ja: "スキル発動フィルタ(Stand/Action/Timing)の遵守", ko: "스킬 발동 필터(Stand/Action/Timing) 준수" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Opponent-side value modifiers", ja: "相手側の計算値補正(相手の~ ActName)", ko: "상대측 계산값 보정(相手の~ ActName)" },
    status: "assumed",
  ...
  },
  {
    ...
    label: { en: "Raw-stat ActNames bypass hooks", ja: "生の能力値ActName(力・魔力等を直接補正)", ko: "원시 스탯 ActName(힘·마력 등 직접 보정)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "skills.sync-sids",
    label: { en: "SyncSids/SyncConditions expansion", ja: "SyncSids・SyncConditionsの展開", ko: "SyncSids·SyncConditions 전개" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Unknown condition identifiers fall back to not-applied", ja: "未知の条件識別子 = 未適用に降格", ko: "미지 조건 식별자 = 미적용 강하" },
    status: "assumed",
  ...
  },
  {
    ...
    label: { en: "Style-variant skill branches", ja: "スタイル分岐スキル(兵種別の変種)", ko: "스타일 분기 스킬(병종별 변형)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Orphan SIDs (event-granted, no static source)", ja: "孤児SID(静的ソース無し — イベント付与と推定)", ko: "고아 SID(정적 소스 없음 — 이벤트 부여 추정)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Skill collection sources (12 categories)", ja: "スキル収集ソース(Categorys 12種)", ko: "스킬 수집 소스(Categorys 12종)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Duplicate rejection and Priority override", ja: "重複排除・Priorityによる上位置換", ko: "중복 배제·Priority 상하위 교체" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Layer exclusive slots (A/B/C/D)", ja: "Layer排他スロット(A/B/C/D)", ko: "Layer 배타 슬롯(A/B/C/D)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Combat-flow hooks (damage, counts, exp, ...)", ja: "戦闘フローのフック(ダメージ・回数・取得経験等)", ko: "전투 흐름 훅(ダメージ·回数·取得経験 등)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Aura operations (AroundName/Operation/Value)", ja: "オーラ演算(AroundName・Operation・Value)", ko: "오라 연산(AroundName·Operation·Value)" },
    status: "absent",
    evidence:
  ...
  {
    id: "skills.work-ops",
    label: { en: "Work-field operations", ja: "Workフィールド演算(5種)", ko: "Work 필드 연산(5종)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "WeaponLevel merged by max per slot", ja: "武器レベル補正 = スロット別の最大値でマージ", ko: "무기 레벨 보정 = 슬롯별 최댓값 병합" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "EnhanceLevel = summed gate (>=1)", ja: "EnhanceLevel = 合算1以上のゲート", ko: "EnhanceLevel = 합산 1 이상 게이트" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Lueur +5 crit = Vander's aura (identified)", ja: "リュール必殺+5 = ヴァンドレの白の忠誠オーラ(解明済み)", ko: "뤼에르 필살 +5 = 반데르 白の忠誠 오라(규명 완료)" },
    status: "absent",
  ...
  // ── 엠블렘·인게이지 ──
  {
    ...
    label: { en: "Sync stat bonuses via skills", ja: "シンクロの能力値補正(スキル経由)", ko: "싱크로 스탯 보정(스킬 경유)" },
    status: "anchored",
    evidence: "corpus.test.ts 기본능력 표시 = 싱크로 칩 포함",
  },
  {
    id: "emblem.sync-bond-level",
    label: { en: "Sync skills = union of bond levels 1..N (highest per series)", ja: "シンクロスキル = 絆1..Nの和集合・同系統は最高レベル", ko: "싱크로 스킬 = 絆 1..N 합집합·동계열 최고 레벨" },
    status: "anchored",
    evidence:
      "絆3 실측 정합(fe17.test.ts 技+2·ブレイク時追撃 — gaps/C §4-3) · 레벨값은 편집기(M4) 소유(기본 = god Level) · ★IL2CPP로 규칙 확정(5.0.0, 2026-08-17, il2cpp/EMBLEM_ENGAGE.md §6): GodGrowthData.OnCompletedEnd(RVA 0x2332320)가 레벨별 누적 LevelData를 사전 생성하고 LevelData.Add(RVA 0x1CD7500)가 병합을 수행 · **동계열의 정본 = SkillData.Group(+0x90)** — GroupAssign(RVA 0x248D0C0)이 skill.xml 習得優先度 연속 구간으로 부여(100그룹·412스킬) · 대체 게이트 = PrivateFlags.CanOverride(Priority<=99), 승자 = Priority 큰 쪽 · Priority=0은 그룹이 없어 항상 합집합(SID_ブレイク時追撃) · ★배선 완료(2026-08-18 4b, fe17.ts): SID 정규식 근사 폐기 → GroupAssign 로드 시 재현(전수 100그룹·412스킬 일치)·LevelData.Add 병합 규칙 이식 · 인게이지 중 EngagedSkills 교체 + EngageSid 치환도 배선(emblemEngagedSids·엔진 effectiveSkills, fe17.test.ts 에이리크 腕輪 치환·대체)",
  },
  {
    id: "emblem.chapter-bond-level",
    label: { en: "Per-chapter emblem bond level baseline", ja: "章ごとの紋章士の絆レベル基準値", ko: "챕터별 엠블렘 絆 레벨 기본값" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage activation, meter, duration", ja: "エンゲージ発動・カウント・持続", ko: "인게이지 발동·카운트·지속" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Engage weapons and engage skills", ja: "紋章士の武器・エンゲージ技", ko: "엠블렘 무기·인게이지 기술" },
    status: "deferred",
    evidence:
  ...
  {
    id: "emblem.bond-ring",
    label: { en: "Bond rings (stats, S-rank skills)", ja: "絆の指輪(能力値補正・Sランクスキル)", ko: "絆지환(스탯 보정·S랭크 스킬)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Skill inheritance (cost, availability)", ja: "スキル継承(コスト・可否)", ko: "스킬 계승(비용·가능 여부)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Cannon (bullet) weapons: dedicated hit formula and range falloff", ja: "砲弾武器専用の命中式と距離減衰", ko: "포탄 무기 전용 명중식과 거리 감쇠" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Weapon/item granted skills (EquipSids)", ja: "武器・アイテム付与スキル(EquipSids)", ko: "무기·아이템 부여 스킬(EquipSids)" },
    status: "absent",
    evidence: "수집 결손 85행(items.json에 EquipSids 259행 이미 산출 — 배선만 없음, gaps/I 투자 2위)",
  },
  {
    id: "weapons.add-effect-schema",
    label: { en: "Item add-effect schema (AddTarget x AddRange x AddType x AddPower x AddSids)", ja: "アイテム付加効果スキーマ(AddTarget×AddRange×AddType×AddPower×AddSids)", ko: "아이템 부가효과 스키마(AddTarget×AddRange×AddType×AddPower×AddSids)" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Enchant applies to all weapons sharing a name, lasts until map end", ja: "エンチャント = 同名武器すべてに適用、マップ終了まで持続", ko: "인챈트 = 동명(同名) 무기 전체 적용, 맵 종료까지 지속" },
    status: "anchored",
  ...
  },
  {
    ...
    label: { en: "Tonic items scale stat gain by unit level", ja: "トニック系アイテム = ユニットのレベルに比例した能力値上昇", ko: "토닉류 아이템 = 유닛 레벨에 비례한 스탯 상승" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Forging and engraving bonuses", ja: "錬成・刻印の補正", ko: "연성·각인 보정" },
    status: "absent",
    evidence:
  ...
  {
    id: "turn.phase-cycle",
    label: { en: "Phase cycle and turn increment", ja: "フェイズ循環(生存軍のみ)・自軍に戻るとターン加算", ko: "페이즈 순환(생존 군만)·자군 복귀 시 턴 증가" },
    status: "implemented",
    evidence: "battle.test.ts · 배너 문자열 4군 순환 방증(gaps/E §1-9)",
  },
  {
    id: "turn.activation-reset",
    label: { en: "Phase start resets acted/moved/broken", ja: "フェイズ開始時に行動・移動・ブレイクをリセット", ko: "페이즈 복귀 시 행동·이동·브레이크 리셋" },
    status: "implemented",
    evidence:
      "battle.test.ts · ☠**리셋 대상은 다음 페이즈 진영뿐**이다(engine battle.ts endPhase `aged.force !== nextForce` 조기 반환) ⇒ 자군 페이즈 동안 적 유닛은 직전 적 페이즈의 `acted === true`를 그대로 들고 있다. **소비층이 이 사실을 모르면 조용히 틀린다** — 실측 2026-08-22: (1) 기보 정책의 종전 위험지대가 그 적을 부동으로 봐 위험을 크게 과소평가했다(m003 턴2 386칸 → 819칸, 위임으로 해소) · (2) UI 위협 색인(apps/web/src/lib/threat.ts `activeFoes`)은 `!u.acted`로 **그 적을 위험 범위에서 통째로 뺀다**(턴 2부터 발현, 미해소). ★미해소 사유 = 정본 플래그가 페이즈 단위인지 턴 단위인지 미판독이고, 바꾸면 아크 규칙(`CanActWithoutEngageCharge`)까지 뒤집혀 사용자 판단이 필요하다. 제거 조건 = `Unit.CanAct` 리셋 시점 판독 또는 실기 관측(턴 2 자군 페이즈 ZR)",
  },
  {
    id: "turn.terrain-heal",
    label: { en: "Terrain heal/damage at turn start", ja: "地形による回復・ダメージ(ターン開始時)", ko: "지형 회복·피해(턴 시작)" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Chapter hold level (Fell Xenologue)", ja: "章の固定レベル(邪竜の章 HoldLevel)", ko: "챕터 고정 레벨(사룡의 장 HoldLevel)" },
    status: "absent",
  ...
  {
    id: "turn.rewind",
    label: { en: "Time crystal rewind (limits, script gating)", ja: "巻き戻し(竜の時水晶 — 回数制限・スクリプト制御)", ko: "되감기(시간의 수정 — 한도·스크립트 통제)" },
    status: "absent",
  ...
  },
  {
    ...
    label: { en: "Map gimmicks (spread, hazards, collapse)", ja: "マップギミック(拡散・危険マス・崩落等)", ko: "맵 기믹(확산·위험타일·붕괴 등)" },
    status: "absent",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Rout victory / player wipe defeat", ja: "敵全滅 = 勝利・自軍全滅 = 敗北", ko: "적 전멸 = 승리 · 자군 전멸 = 패배" },
    status: "implemented",
    evidence: "battle.test.ts",
  },
  {
    id: "turn.victory-objectives",
    label: { en: "Chapter-specific objectives", ja: "章固有の勝利条件", ko: "챕터 고유 승리 조건" },
    status: "deferred",
  ...
  },
  {
    ...
    label: { en: "Reinforcements (turn/condition spawns)", ja: "増援(ターン・条件で出現)", ko: "증원(턴·조건 등장)" },
    status: "deferred",
    evidence: "Lua 이벤트 엔진(§0 미룸) — 증원 5패턴, Dispos() 단일 원시함수 363회 전량·반례 0(gaps/E §2-1, M §5)",
  },
  {
    id: "turn.enemy-ai",
    label: { en: "Automatic enemy phase AI", ja: "敵フェイズAIの自動進行", ko: "적턴 AI 자동 진행" },
    status: "assumed",
    evidence:
  ...
  {
    id: "turn.delegate",
    label: { en: "Delegate (auto-battle)", ja: "おまかせ(自動進行)", ko: "위임(자동진행)" },
    status: "deferred",
    evidence:
  ...
  // ── 적 AI 데이터 ──
  {
    ...
    label: { en: "AI routine opcode programs", ja: "AIルーチン(オペコードプログラム141種)", ko: "AI 루틴(옵코드 프로그램 141종)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "AI field projection completeness", ja: "AIフィールド射影の完全性", ko: "AI 필드 사영 완결성" },
    status: "implemented",
    evidence:
  ...
  {
    id: "ai.move-limit",
    label: { en: "AI movement boxes (hard constraint)", ja: "AI移動制限ボックス(AI_MoveLimit)", ko: "AI 이동 제한 박스(AI_MoveLimit)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Band-linked AI activation", ja: "バンド連動の起動(AI_BandNo)", ko: "밴드 연동 기동(AI_BandNo)" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Conditional AI routine swap", ja: "条件付きサブAIの差し替え", ko: "조건부 서브 AI 치환" },
    status: "implemented",
    evidence:
  ...
  },
  {
    ...
    label: { en: "AI opcode interpreter (slot order, Active gates)", ja: "AIオペコードインタプリタ(スロット順・Activeゲート)", ko: "AI 옵코드 인터프리터(슬롯 순서·Active 게이트)" },
    status: "implemented",
    evidence:
  ...
  {
    id: "ai.move-hero",
    label: { en: "MV_Hero (89): chase the protagonist", ja: "MV_Hero(89) — 主人公を追跡", ko: "MV_Hero(89) — 주인공 추격" },
    status: "anchored",
    evidence:
  ...
  {
    id: "ai.mind-village",
    label: { en: "MI_Village (62): head for villages (to destroy)", ja: "MI_Village(62) — 民家へ向かう(破壊目的)", ko: "MI_Village(62) — 민가로 향한다(파괴 목적)" },
    status: "assumed",
    evidence:
  ...
  },
  {
    ...
    label: { en: "AI target scoring (bitfield lexicographic)", ja: "AIの標的評価(ビットフィールドの辞書式比較)", ko: "AI 표적 평가(비트필드 사전식 비교)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "events.triggers",
    label: { en: "Event triggers (inspectors)", ja: "イベントトリガ(インスペクタ26種)", ko: "이벤트 트리거(인스펙터 26종)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "events.dispos",
    label: { en: "Event spawn (Dispos) & initial placement", ja: "イベントスポーン(Dispos)・初期配置", ko: "이벤트 스폰(Dispos)·초기 배치" },
    status: "anchored",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Dispos difficulty mask (Flag bits N/H/L)", ja: "dispos難易度マスク(Flag N/H/Lビット)", ko: "dispos 난이도 마스크(Flag N/H/L 비트)" },
    status: "anchored",
    evidence:
  ...
  {
    id: "events.variables",
    label: { en: "Game variables (GameVariable)", ja: "ゲーム変数(GameVariable)", ko: "게임 변수(GameVariable)" },
    status: "assumed",
    evidence:
  ...
  {
    id: "events.win-rules",
    label: { en: "Win/lose rules", ja: "勝敗規則(章固有)", ko: "승패 규칙(챕터 고유)" },
    status: "assumed",
    evidence:
  ...
  },
  {
    ...
    label: { en: "Event AI rewrite (record only)", ja: "イベントAIの再設定(記録のみ)", ko: "이벤트 AI 재설정(기록만)" },
    status: "deferred",
    evidence:
      "AiSetSequence(코퍼스 369회 — 게임플레이 API 최다)·AiSetActive·AiSetRejectPower0Attack·AiClearMoveLimit = 유닛 aiScript에 원문 인자 기록 + ai 이벤트(결정 2026-08-18: 소비 = MP4 AI 실행기) · ⚠AiGetActive 질의는 false 강하(미소비 상태 — MP4에서 실상태로 교체) · ★부분 소비 개시(2026-08-22): `AiSetRejectPower0Attack`만 배선했다 — engine ai/unit.ts `rejectsPower0`가 유닛 aiScript의 **마지막 호출을 난이도 강제·dispos 플래그보다 우선**한다. 정본이 루나틱 강제를 Lua로 되돌리는 이유가 스크립트에 적혀 있다(ハマり = 교착). 코퍼스 전수 = e001(루나틱·적 전원)·m020(보스)·g002(증원 PID 7종)·common(비행 적, ☠호출자가 common_p0뿐이라 챕터 밖) · g005는 주석 사문. 관통 테스트 = engine tests/events.test.ts(Lua → aiScript → rejectsPower0). ☠나머지 3종(Sequence·Active·ClearMoveLimit)은 여전히 기록만",
  },
];
