...
- ★**웹 작업 내부 루프 = `./dev check:web`**(apps/web tsc + vitest, 10.5초 · 로컬 dev 서버 무해) —
  `./dev gate`는 **커밋 직전 1회**만. ☠게이트 안의 `astro check`와 `tools/rulebook/build.mjs`가
  `apps/web/node_modules/.vite`를 갈아엎어 **열려 있는 dev 서버를 깨뜨린다**(500 또는 하이드레이션 사망
  = 커스텀 드롭다운 전부 무반응). 게이트는 스스로 되살리지만(run_guarded) 그 15초가 매번 든다
- 상시 커밋(자유) — 결정 즉시 파일로 쓰고 커밋. ☠**./dev done = 마감 1개 명령**(게이트 → main 병합 → main 푸시 = 베타 게시 → beta 동기 → 잔류 프로세스 정리(dev 서버·좀비 에이전트) → /clear 손실 판정) — **M단위 마감에만**. 작업 중 게이트는 `./dev gate`
...
- ★**기보 확인은 로컬 서버로**(2026-08-19 사용자 지시) — 기보를 만들거나 고치면 **`./dev web`**(정지 → `.vite` 삭제 → 재기동 → 200 확인까지 한 명령. ☠vite SSR 사전번들 재최적화 500의 정본 복구 절차다)으로 올리고 `http://localhost:4321/ko/fe17/maps/<cid>` 주소를 함께 보고한다. ☠**베타 빌드는 `./dev done`에 물려 있어 확인 루프로 쓰기엔 느리다** — 마감에만 쓴다. 절차 정본 = `run-web` 스킬(타일 셀렉터·호버 선행·함정). ☠엔진·스토어 코드를 고친 뒤에는 **서버 재기동**이 필수다(vite 의존성 404로 보드가 무반응이 되는데 에러가 안 뜬다).
- 배포: **main 머지 = 베타 게시(스테이블 불변) · 스테이블 = 명시 지시 시 ./dev promote** —
  ☠채널 주소·흐름·CF 설정의 정본 = `rules/deploy.md` (규약이 커지면 rules/에 주제별 분리)
- ☠★**기보는 사용자 지시 전에 만들지 않는다**(2026-08-22 사용자 지시 — 2026-08-18의 "룰이 바뀌면 다시 만든다"를 **대체**).
  **엔진 피델리티가 최우선**이고 기보는 그 산물이다. 룰이 바뀌면 저장분은 낡은 판의 기록이 되지만
  **낡았다는 사실을 등재할 뿐 재생성하지 않는다** — 피델리티가 계속 오르는 구간에서 다시 만들면 매번 다시 만든다.
  지시가 오면 사슬을 앞 장부터 잇는다(`./dev replay <cid> --seed <n> --carry <앞 기보>`).
  ★따라서 **실기 앵커를 기보에 매달지 않는다**(기보가 바뀌면 앵커가 사라진다 — 2026-08-22 실제 발생).
  앵커는 기보 비의존 고정 시나리오로 박는다
- ★**현지화(ja 등 번역)는 사용자 지시 시에만 — 마감 게이트에 물리지 않는다**(2026-08-31).
  콘텐츠성 라벨은 ja 선택 필드 + en 폴백 · UI 문자열(lib/i18n.ts)만 전 로케일 필수(소량이라 비차단)
