...
추가 결정 (2026-08-31):
- ★**내부 레벨 = 0기점 정본**(`内部レベル計算` = clamp(base + 레벨 − 1, 0, L50), base = person → job 폴백) — 1기점 규약 폐기. **사용자 실기 앵커 = 모브 20 + 12 − 1 = 31**(triangleattack 위첨자 동일). 종전 1기점 + person 단독이 모브 스탯 이상의 원인이었다
- ★**DLC 제외 해제**(사용자 지시) — 로스터 +5(사룡의 장: 엘·라팔·셀레스티아·그레고리·마델린, E챕터 joins가 빈 배열이라 명시 예외로 편입) · 직업 +2(인챈트·메이지캐넌) · **성옥의 가호 체커**(星玉の加護 = Work 3 TotalGrowChange +15 — 데이터 행 그대로 소비, 엔진 calcWork Work 3 훅 배선). ☠결손 1건 = 라팔 얼굴 스프라이트가 facethumb 번들에 부재(테스트로 박제)
- **표 헤더 = 영문(HP/STR/…/BLD) + JetBrains Mono**(사용자 지시 — 빌더 페이지만 폰트 로드)
...
**내부 레벨 표기 규약**: ☠1기점 규약은 2026-08-31 폐기 — **0기점 정본 그대로**(§1 추가 결정, 모브 앵커 31). 표시·목표·경로 계산 전부 `内部レベル計算` 값(clamp 0..50)을 쓴다.
...
전건 종결(2026-08-31): 질문 게이트 Q1~Q4 = §1 흡수 · 전용직 불가 행 = **하단 회색 확정**(사용자 화면 확인 "완벽하다") ·
인챈트·메이지캐넌 = DLC 해제로 **포함** 확정
