...
## 7. 챕터 베이스라인 (2026-08-18 사용자 요청 — ☠후순위 확정)

> **"완주하고 나면 맵별 누적사항을 기록하여 평균값을 내어, 별도 챕터만 할 수 있는 베이스라인을
> 각 챕터마다 등록. 중간 챕터에서 플레이를 수정 주문할 때 뒷내용도 바꿔야 하는 것을 미연에 방지.
> 완주기록과 별도로 취급."**

### 7-1. 문제 — 사슬은 수정에 약하다

인계(§4)는 챕터를 **직렬로** 묶는다. 그래서 5장 기보를 고치면 6장 이후의 진입 로스터가 전부 달라져
**뒤 전부를 다시 만들어야 한다.** 챕터가 54개인 자산에서 이 결합은 수정 비용을 곱셈으로 키운다.

### 7-2. 해법 — 베이스라인 = 챕터의 표준 진입 상태

각 챕터에 "이 장을 단독으로 시작할 때의 표준 로스터"를 **하나 고정해 등록**한다.
그러면 5장 기보를 고쳐도 6장은 자기 베이스라인 위에 서 있으므로 **다시 만들 필요가 없다.**

- **그릇 = `EphemerisSetup`**(pid 키 `SetupUnit`) — 인계 로스터와 **같은 형태**다(5-2에서 이미 열었다).
  새 스키마를 만들지 않는다: 소비처(`projectUnit`)도 그대로다.
- **저장 = `data/fe17/baselines/{cid}.json`** — ☠기보(`data/fe17/replays/`)와 **다른 디렉터리·다른 수명**.
  이것이 "완주기록과 별도 취급"의 실체다: 기보를 고쳐도 베이스라인은 **자동으로 안 바뀐다**(명시 재생성 시에만).
- **우선순위**: 런(사슬)이 진행 중이면 런 로스터가 이긴다 · 단독 진입이면 베이스라인 · 둘 다 없으면 dispos 기본.

### 7-3. 산출 — 완주 N회의 평균

완주 런에서 **각 챕터 진입 시점**의 자군 로스터를 뽑아 챕터별로 모으고 평균을 낸다.
☠no-fiction: 평균에는 **표본 수를 반드시 병기**한다(`samples: N`).

☠**평균은 그대로 쓰면 모순을 낳는다.** 스탯·레벨은 정수이고 고정 성장(§3)은 결정적이라,
평균 스탯이 "그 레벨에서 도달 가능한 값"이 아닐 수 있다(누적기·baseCap과 어긋난다).
⇒ **정합화 규칙이 필요하다** — 아래 §미결.

### 7-4. 선행 조건 ☠

- ★**엔딩까지 전 챕터 완주를 몇 차례 반복해야 한다**(2026-08-18 사용자 확정) — 평균은 표본이 있어야 선다.
  현재 정책 플레이어는 m003도 자군 1명을 잃는다(5-5 실측) ⇒ 표본 0. 공급원은 **MA 트랙(모범답안지)의 완주 기보**다.
- 전직(5-4)이 서야 중후반 챕터의 로스터가 의미를 갖는다(상급직 진입 시점이 스탯을 지배한다).

### 7-5. 단계 (착수 지시 후)

- 7-a `carryover`를 **챕터 진입 시점**에도 돌려 런 기록에 남긴다(현재는 종료 시점만)
- 7-b 베이스라인 산출기 = `./dev baseline <cid>`(표본 = replays/ 완주 사슬 N개) → `data/fe17/baselines/{cid}.json`
- 7-c 소비 배선 = 맵 단독 진입 시 setup 주입(런 > 베이스라인 > dispos)
- 7-d UI = 이 챕터가 베이스라인 위에서 시작함을 표시(무엇을 근거로 선 판인지 숨기지 않는다)
...
- ★**베이스라인 평균의 정합화 규칙**(§7-3) — 착수 시 사용자 결정. 후보:
  (A) 레벨만 평균·반올림하고 **스탯은 고정 성장 모델로 재산출**(누적기·baseCap과 모순 없음 — 권장) ·
  (B) 스탯도 평균·반올림(표본에 충실하나 도달 불가 조합이 생긴다) ·
  (C) 평균에 가장 가까운 **실제 런 1개를 대표로 채택**(항상 도달 가능하나 "평균"은 아니다).
  안 정하면 산출기(7-b)를 못 쓴다.
- 베이스라인 표본 수 하한(N=1도 등록하나, 몇 개부터 신뢰하나) — 7-b 착수 시.
