...
- 컨트롤: 직업 드롭다운(한글, 상급직만) + 내부 레벨 드롭다운(디폴트 40) [+ 난이도 = §7 Q1]
...
- 구성은 §7 Q4 (권장 = 간판형: 한 줄 소개 + 카드 3장 — 시뮬레이터(맵 목록)·캐릭터 빌더·재현 상태(fidelity))
...
## 7. 미결 (질문 게이트 — 사용자 결정 대기)

- **Q1 난이도**: 합류 초기치가 난이도별(Offset N/H/L)로 다르다. A) 루나틱 고정(권장 — MA 트랙·기존 검증과 정합, 셀렉터는 후속 확장) B) 셀렉터 3종 동시 제공 C) 노멀 고정. 안 정하면 = 초기치 표가 어느 실기와도 대조 불가.
- **Q2 레벨 드롭다운 범위**: A) 10~50, 5단위(권장 — 촘촘함과 목록 길이 절충) B) 1~50 전체 C) 20/30/40/50만. 안 정하면 = UI 확정 불가(경미).
- **Q3 전용 상급직 포함**: A) 범용 상급직만(권장 — "상위직만" 지시의 보수 해석, 전용직은 주인 캐릭터 외엔 무의미) B) 전용직 포함(선택 시 비주인 캐릭터 행 비활성). 안 정하면 = 드롭다운 목록 확정 불가.
- **Q4 메인페이지 구성**: A) 간판형 — 소개 + 카드 3장(권장 — 현 콘텐츠 규모에 맞고 열람 게이트에 유리) B) 허브형 — triangleattack 홈처럼 섹션별 링크 목록(콘텐츠가 늘면 전환). 안 정하면 = B4 착수 불가.
