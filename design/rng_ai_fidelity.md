...
## 3A. B4 재측정 결과 (2026-08-18 완료 — 시드 42 고정, 전 54챕터)

☠**종전 98.8%는 폐기**한다. 유령 유닛(비자군 1,624 과배치·증원 조기 배치) 위에서 잰 값이었다.

| 지표 | 값 |
|---|---|
| 측정 성립 챕터 | **45 / 54** (9챕터는 개시조차 못 한다 — 아래 §3A-2) |
| `verified` | **45 / 45** (기록·재생 계약은 전부 성립) |
| 비자군 유닛 합계 | 1,786 |
| 결손 유닛 | 78 → **자동화율 95.63%** |
| 결손 건수 | 461 |
| 결손 0 챕터 | 22 / 45 |

### 3A-1. 결손 461건의 정체 — **미판독 함수 하나가 58%다**

| 건수 | 원인 |
|---|---|
| **268** | ★`GetSidePosition`(0x195FC80) 미판독 — 가드 위치 규칙. `AI_AT_Attack` 115 · `HealToAttack` 67 · `AttackToHeal` 42 · `Enchant` 38 · `Heal` 22 외 |
...
| 5 | `HealMindToTerrain` 게이트(Hc_Terrain) 미판독 |
| 5 | 엔진(페이즈 미진행·1000액션 초과) |

⇒ **B 트랙 우선순위 확정**: `GetSidePosition` 판독 1건이 결손의 58%를 지운다. 그다음이 `AI_MI_Village`.

### 3A-2. 개시 불가 9챕터 — MP2(이벤트 엔진) 선행 결함

☠**전부 MP3 3-7 수리 이전 커밋(1583cd0)에서도 같은 오류로 재현된다** = 회귀가 아니라 선행 결함이다.

| 원인 | 챕터 | 증상 |
|---|---|---|
| 소지품 계약 | e005 · e006 · m014 | `UnitPutOffItem: 소지품 아님 IID_ドラゴンキラー` / `IID_リベラシオン` · `UnitSetItemEquip: 소지 무기 아님` — 자군 소지품이 dispos 근사라 스크립트가 기대하는 물건이 없다(MP5 인계가 서면 해소될 가능성) |
| Lua 문법 | g001 · g004 | `'end' expected (to close 'if')` — 우리 전처리(주석 제거·`!=` 정규화·식별자 맹글링)가 특정 구문을 깨뜨린다 |
| 스크립트 로드 | g002 · g003 · g005 | `G00x_Gimmick` 미수록 — 파이프라인이 이 소스를 안 싣는다 |
| 페이즈 위반 | m001 | `u7는 지금 군의 유닛이 아니다` — `UnitTransfer` 뒤 재생 검증이 force 변경을 못 따라간다 |
